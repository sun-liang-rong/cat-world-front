...
- 超级挑战：首页左侧金币入口，对标“羊了个羊第二关”的固定单关——约 50 张牌 / 15 种元素 / 6～7 层深堆、收集槽只有 5 格（额外槽位道具可 +1），贴满压力规划让最优路径长期骑在 4/5 满槽边缘（无放松段、按残酷度挑选候选）；每日首次通关发大额金币、同日重复通关按普通过关金币发放（入口文案切换为“今日已领取”）；不发星星、不写入难度自适应，失败“再玩一次”保持难度曲线只换元素排布
...
- `DifficultyController.ts` 保存最近 10 局表现，综合胜率、连胜/连败、失误、耗时和槽位余量计算下一关目标，并提供连续失败救援和三步恢复。在其之上有一层按关卡号驱动的“节拍表”：主题内第 5/10/15/20 关为 breather 爽关，第 9/14/19 关为 spike 难关（连败 ≥ 2 次自动取消，救援关优先于节拍表），让难度曲线呈现“爬坡—释放—小高潮”的受控起伏。
...
该脚本覆盖新档默认值、奖励与扣款、建筑建设和猫咪解锁、商店每日限购、每日任务与宝箱、猫咪互动、超级挑战领奖标记、无尽纪录以及旧档/坏档归一化。
