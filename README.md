...
- 超萌挑战：首页左侧金币入口，对标“羊了个羊第二关”的固定单关——约 50 张牌 / 15 种元素 / 6～7 层深堆、收集槽只有 5 格（额外槽位道具可 +1），贴满压力规划让最优路径长期骑在 4/5 满槽边缘（无放松段、按残酷度挑选候选）；每日首次通关发大额金币、同日重复通关按普通过关金币发放（入口文案切换为“今日已领取”）；不发星星、不写入难度自适应；每次进入或再玩一次都保持高压配置、只换元素排布，避免记住卡牌位置
